...
class BlogListView(APIView):
    ...
    def get(self, request, format=None):
        if Post.postobjects.all().exists():
            ...
            return Response({'error': 'No hay Publicaciones'}, status=status.HTTP_404_NOT_FOUND)
...
class ListPostsByCategoryView(APIView):
    ...
    def get(self, request, format=None):
        if Post.postobjects.all().exists():
            ...
            return Response({'error': 'No existe publicaciones'}, status=status.HTTP_404_NOT_FOUND)
...
class PostDetailView(APIView):
    ...
    def get(self, request, slug, format=None):
        if Post.postobjects.filter(slug=slug).exists():
            ...
            if not ViewCount.objects.filter(post=post):
            ...
            return Response({'error':'No exite la publicacon'}, status=status.HTTP_404_NOT_FOUND)
